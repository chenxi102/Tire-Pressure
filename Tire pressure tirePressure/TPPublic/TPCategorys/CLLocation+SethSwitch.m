...
@implementation CLLocation (SethSwitch)
...
- (CLLocation *) locationEarthFromMars {
    ...
    double Earthlat = 0.0 , baidulat = 0.0 , marslat = 0.0 , templat = 0.0;
    double Earthlng = 0.0 , baidulng = 0.0 , marslng = 0.0 , templng = 0.0;
    // 1. 火星转百度 得到百度的坐标。
    transform_mars_2_bear_paw(self.coordinate.latitude, self.coordinate.longitude, &baidulat, &baidulng);
    // 2.  baidu 转 earth  :  2*baidu - (把百度当成gps 转火星 火星转百度)
    transform_earth_2_mars(baidulat, baidulng, &marslat, &marslng);
    transform_mars_2_bear_paw(marslat, marslng, &templat, &templng);
    Earthlat = 2*baidulat - templat;
    Earthlng = 2*baidulng - templng;
    return [[CLLocation alloc] initWithCoordinate:CLLocationCoordinate2DMake(Earthlat, Earthlng)
                                         altitude:self.altitude
                               horizontalAccuracy:self.horizontalAccuracy
                                 verticalAccuracy:self.verticalAccuracy
                                           course:self.course
